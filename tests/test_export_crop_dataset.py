...
1. **The labels live in the filenames.** `007mz25c_-_118_596_-_478_611.jpg` is panorama
   `007mz25c` with keypoints (118, 596) and (478, 611), and the export's whole value-add is
   turning that convention into a real column. If `parse_name` silently drops a keypoint, the
   published dataset is quietly wrong in a way no byte-level check would catch -- the image
   bytes still round-trip perfectly.
...
from export_crop_dataset import keypoint_summary, parse_name  # noqa: E402


def test_parses_pano_id_and_a_single_keypoint():
    pano_id, keypoints = parse_name("007mz25c_-_118_596")
    assert pano_id == "007mz25c"
...
def test_pano_id_survives_a_crop_with_no_keypoints():
    pano_id, keypoints = parse_name("007mz25c")
    assert pano_id == "007mz25c"
    assert keypoints == []
