# Location precision assessment (#96, #103): the gate on every route to 500k

§5–§5o of [`curb_ramp_data_sourcing.md`](curb_ramp_data_sourcing.md), moved verbatim under #145
with their section numbers kept, so a reference to "§5g" still means the same section once its
file name is updated. A bare § reference to §0–§0c means [`data_scaling_59.md`](data_scaling_59.md),
and one to §1–§4 or §6–§10 means `curb_ramp_data_sourcing.md`. This file is ~1,500 lines by
...
1. **The government coordinates survive into the published dataset verbatim.**
   ...
   is what made the analysis possible before the original portal files were recovered; they are now
   committed at `stage_one/dataset_generation/location_data/` (§9), so it no longer depends on that.
