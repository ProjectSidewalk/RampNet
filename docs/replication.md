...
| `analysis_out/compute_log.jsonl` (cluster GPU-hours + $) | ~2 MB, grows per pull | **committed** ✅ | ⚠️ needs a `sacct` pull per cluster (#143); klone back-filled 2026-08-19 (3,990 rows) from the committed dump `docs/data/compute/sacct_klone_2026-08-19.txt` (748 KB, sha256 pinned in `compute_cost.md`); Tillicum back-filled 2026-09-21 (38 rows, $607.24) from `docs/data/compute/sacct_tillicum_2026-09-21.txt` (7.3 KB, sha256 pinned the same way), reconciled to `hyakusage` to the cent |
