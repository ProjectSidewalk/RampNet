...
| `benchmark/train_overlap.json` | **required** — the exporter refuses a split without an entry. Re-run `python scripts/analysis/train_overlap_check.py` (network read-only, minutes) and commit the result; it fills the `records` config's `train_overlap` column (#127) |
