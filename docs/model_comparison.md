...
starts at 0.65–0.78 at epoch 1 (the pre-collapse high across the six configurations; every
run's global maximum comes after the recovery), is already lower at epoch 2 in every one,
and bottoms at 0.00–0.25 between epochs 3 and 6, while training `cls_loss` stays flat (at
most +0.07 above its epoch-2 value over epochs 3–6, below epoch 1 by epoch 8) and
validation `cls_loss` rises to 1.7–7.3× its epoch-1 value; nothing fails numerically (no
NaN or Inf in any committed CSV). It is a recall collapse, not a false-positive flood —
...
hardware (minimum 0.10–0.12 at epoch 3), so all nine independent runs show it (eleven
committed curves, two of them continuations that repeat their parents' early epochs). The
reported checkpoints all sit long after the recovery (`y11l_pano` ep59, `y26_pano` ep56,
`y11x_pano_h200` ep60, `y11x_tiles` ep44 — two of these are later than the committed
curve that shows their recovery: `y26_pano/results.csv` ends at epoch 55 and
...
Moved verbatim to [`vistas_transfer_126.md`](vistas_transfer_126.md) (#145).

## Status

Moved verbatim to [`running_model_comparison.md`](running_model_comparison.md) § "Status" (#145).

## Gemini credentials

Moved verbatim to [`running_model_comparison.md`](running_model_comparison.md) § "Gemini credentials" (#145).

## Claude on Vertex and first-party (#122, #156)

Moved verbatim to [`claude_legs_122.md`](claude_legs_122.md) (#145): the first annapolis results, Claude Fable (#156), reproducing the legs, and splitting a two-leg day by effort. The wiring notes, "Claude on Vertex (#122): two constraints worth knowing before you wire it up", are in [`running_model_comparison.md`](running_model_comparison.md).
...
Moved verbatim to [`running_model_comparison.md`](running_model_comparison.md) § "Running it" (#145).

## Running the open-weight models on Hyak

Moved verbatim to [`running_model_comparison.md`](running_model_comparison.md) § "Running the open-weight models on Hyak" (#145).
...
Moved verbatim to [`running_model_comparison.md`](running_model_comparison.md) § "Files" (#145).
