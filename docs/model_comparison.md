...
| model | provider | density | joined | scored below |
| :--- | :--- | :--- | :--- | :--- |
...
**A model can be run, published and verified before its write-up lands**, and the last row is
that case: `gemini-3.7-flash` has run on all ten splits, its detections are published and
