...
the third clause fails.

## Result

**The frozen model does not benefit from more input pixels on any split. At the issue's
forecast point, 2× (r4096), it hurts on 10 of 11 splits.** The one exception, paterson, gains
F1, and u4096 gains as much with no new pixels, so the gain is object scale, not resolution.
Pooled over the eight US splits, r4096 against the 2048×4096 control at 0.30: ΔR −0.068
[−0.087, −0.046], ΔP −0.075 [−0.092, −0.057], ΔF1 −0.071 [−0.086, −0.055]. 1.5× (r3072) comes
closest to harmless: "tolerates" on annapolis, bend and richmond, and ΔR +0.005 [−0.009, +0.019]
pooled, but precision still falls (ΔP −0.047 [−0.061, −0.034]) and pooled F1 with it (−0.018
[−0.028, −0.007]).
...
**That scale shift moves recall in both directions.** Far ramps gain and near ramps lose. Pooled
US at 0.30, r4096 against r2048: recall beyond 25 m goes from 0.543 to 0.750, and within 8 m it
goes from 0.841 to 0.611. By apparent size, ramps under 32 px at the 4096 input gain and ramps of
80 px or more fall from 0.843 to 0.624. A model trained at one object scale finds the ramps that
...
about 3.5°) of x = 0 or x = 1. None are in the top or bottom rows. Nine of the ten op_caches were
written at `c7098be` (2026-07-28), before the seam fix `f4c71c8` (2026-08-18). That fix made
...
op_cache peak is missing, (c) every extra peak is in the strip, and (d) tp/fp/fn at 0.30 and
0.55 are identical. `analysis_out/op_cache` was not regenerated or modified here.
...
- **The op_caches predate the seam fix** (`c7098be`, before `f4c71c8`). They lack the floor peaks
  in the ~3.5° strip beside the seam, so op_cache-based numbers in other docs slightly understate
  both recall and false positives near the seam. This is the same caveat
  [`cascade_cost_35.md`](https://github.com/ProjectSidewalk/RampNet/pull/194) carries.
...
- **Richmond's r4096 "hurts" is on the edge.** The upper bound of its ΔF1 CI is −0.00004, which
  rounds to −0.000. The verdict is taken on the unrounded value. A first version of the report
  rounded before judging and read it as "tolerates".
- **Precision at large inputs is a lower bound.** The GT is anchored to the reviewed 2048-input
  detections, so a real ramp that only a larger input finds scores as a false positive. No
  spot-check gallery of the new FPs was made. Quote ΔP here as "at least this large an apparent
  drop"; a reviewer pass over the r3072 incremental FPs is the obvious follow-up before any
  precision number from this sweep is quoted on its own. The recall and verdict columns do not
  depend on it, and the verdict would be unchanged at r4096 even if every new FP were real,
  because r4096's recall already falls on the pools.
...
| paterson | 0.947 / 0.724 / 0.821 | r3072 | +0.081 [+0.050, +0.115] | +0.043 [+0.023, +0.066] | gains, object scale only |
...
- **paterson is the one split where a larger input raises F1.** The gain is a scale effect: u4096
  does as well, and R(r4096) − R(u4096) is +0.008 [−0.013, +0.028]. Why paterson's ramps sit on
  the side of the scale trade that gains was not investigated.
...
**At a matched 4096×8192 input, native pixels are worth at most about one recall point
(US-pool CI upper bound +0.016), and the point estimate is within noise on every pool.** The
...
only slightly less bad than u4096 on the same panos. The plan asked for a two-way split (native
...
and by apparent size at the 4096-wide input (`apparent_px`, 1.2 m ramp). n is the number of GT
ramps on recall-confirmed panos.
...
**The hypothesis was far-band gains, and the far band does gain.** But u4096 recovers most of that gain with no new information,
and near ramps lose more than far ramps gain. The per-split, headline-pool and GSV-pool band
...
- Whether the new r3072 FPs are real ramps the GT missed (see the precision caveat above).
...
before the script wrote rows on failure, and a one-pano timing profile. The first grid attempt
...
# Full re-run (GPU, ~48 GB card; here an A40): fetch the native panoramas, then the launcher
python scripts/unpack_benchmark_panos.py --out /scratch/bench \
    --cities annapolis,bend,budapest_district5,clovis,gainesville,morgantown,paterson,richmond,sao_paulo
...
and resumes per arm and split. **laurens_gsv and laurens_mapillary are not on the Hub yet**
(`unpack_benchmark_panos.py` docstring), so a clean-clone re-run of those two rows needs the
local bundle imagery. Their caches are committed, so the CPU steps cover them. A GPU with less
free memory than about 30 GB needs `--native-cap` lowered, which changes the rnative arm; state
the cap used. `results.json` from a re-run of `report` was byte-identical on makelab2 (numpy
2.0.2, Linux) and on Windows (numpy 2.5.1).
