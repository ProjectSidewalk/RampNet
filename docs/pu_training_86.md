...
is the part of the 8,674 HF train crops that survives the test exclusions (§3.1: at most 4,275
trainable),
...
All counts are human CurbRamp labels (SidewalkAI's account excluded by user id), from the audit
tables (`analysis_out/ps_audit/*.csv`) on `main`, as regenerated by PR #183 (merged as `29fe638`),
the timestamp fix that keeps 354 labels whose timestamps lack fractional seconds. The tables PR #175 first merged
differ by about 0.1 % (e.g. tag era 361,863 and tagged 118,293 there, against 362,217 and
118,402) and change nothing below; the plan cites the #183 numbers so the scripts written from it
match.
...
hides *missing tactile warning* and *points into traffic*, yet tag-era labels carry them where
unlisted: 772 for *missing tactile warning* (amsterdam and chandigarh-india together; the audit
does not split them) and 809 for *points into traffic* (amsterdam only), audit §4 `tags.csv`.
chandigarh-india lists only 3 tags and hides six of the eight (119 labels). Everywhere else all
eight are listed.
...
few lines in the loss and must not be skipped: amsterdam has 12,258 tag-era labels (`by_city.csv`),
7,096 of them validated correct and so in the tier-2 universe; unmasked, those 7,096 would become
*missing tactile warning* negatives, on the benchmark's strongest tag (AP 0.982,
`tag_benchmark_86.md` §1).
...
with the reason per row, and asserted by the test suite, each by panorama and by the 10 m rule:
training ∩ test = ∅ for each test set, training ∩ slice = ∅, and slice ∩ test = ∅ for each test
set. So a `clean` row within 10 m of a slice row is dropped too, which takes `clean` below the
~3,850 quoted in §3.1 by an amount the slice draw decides.
The four HF train labels with no live row (`tag_benchmark_86.md` §3) have no panorama and no
coordinates, so their distance to the tests is unknown and they cannot be re-cut: they are not
trainable, and at most 4,275 HF train rows are (`plan_numbers.json`, `survivors_trainable`).
...
- Applying §2.4's exclusions to the 8,674 HF train rows removes 3,565 that sit on a panorama of
  ...
  4 of them are the labels with no live row, so **at most 4,275 are trainable**, before the
  expert-validate and review-list exclusions,
...
|---|---|---|---|
| **clean** (control, new run) | the HF train rows that survive §2.4, minus the §5.3 selection slice and its 10 m neighbours: at most 4,275 − ~430 ≈ 3,850 reviewed crops, cut at the §4.1 framing | negative (correct: the ASSETS'24 pass affirmed empty tag sets, audit §8) | the reviewed set alone, matched to the new arms in exclusions, framing and selection rule |
...
The identity needs π_t ≥ o_t. When π_t < o_t (*pooled water* under the proposed prior: π 0.0117,
o 0.0126), the censoring form clamps π_U,t to 0 and becomes `naive`, but the case-control form
...
normalisation. A batch B of size b is drawn uniformly from the whole table of N rows (HF rows
...
chandigarh-india) stays in P_t, while that deployment's untagged cells for the tag are masked out
of U*_t. So U*_t under-represents those deployments relative to P_t, by at most 1,581 of ~296k
cells for two tags. PROPOSED: keep the rule (those positives are real) and state the bias; the
alternative, masking the carried cells too, is a one-line ablation. The
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| **HF train rows that survive §2.4** (proposed; `plan_numbers.json`, `priors.survivors`) | 4,275 | 42.5 % | 13.1 % | 7.9 % | 9.3 % | 6.4 % | 4.2 % | 1.5 % | 1.2 % |
...
| item-3 review, `untagged` stratum (`tag_rubric_draft.md`, PR #176; not yet reviewed) | 150 | a check on π_U,t after reweighting, see below | | | | | | | |
...
**The item-3 review is a check on π_U,t, not a direct estimate of it.** PR #176's review list
samples 150 `untagged` items from 35 deployments, as "where the review measures how often an
untagged label actually carries a condition, which is the number the positive-unlabeled
experiment (plan item 5) needs" (`tag_rubric_draft.md`, Strata). Its raw rate differs from
π_U,t in four ways (same doc, Strata and Composition):

- the stratum is labels with **no tags at all**, while U_t also holds labels tagged with other
  tags (§3.1 treats those as unlabeled for t);
- it is **crop-era only** (placed since 2023-10-12), not the whole tag era;
- it is **not filtered to tier 2** ("Tier 2 ... is not a stratum");
- each stratum's quota is **split equally over deployments**, and draws within a (tag state,
  city) cell rotate through distance bands, so its city and distance mix is the design's, not
  the pool's.

So its raw rate estimates P(t | no tags, crop era) under a city-equalised design. Before it is
compared with π_U,t, it is reweighted: design weights per deployment and distance band from the
eligible-pool counts committed in `review_list.meta.json` (`pool_by_city`,
`pool_by_state_and_band`), then a tier-2 filter on the reviewed items, and
the comparison is stated as covering no-tag labels only. At 150 items its intervals are wide (a
20 % rate is about ±6.4 points at 95 % before design weights, arithmetic), so it checks the prior
rather than replacing it, and it only exists once the review is done. Its 500 items are excluded from
...
Second, where c_t ≥ 1, π_U,t is clamped to 0 and, with π′_t = max(π_t, o_t) (§3.1), nnPU reduces
to `naive` for that tag: with this
...
priors, §7, if the main result is close), and the item-3 `untagged` stratum, reweighted to
the tier-2 pool, checks π_U,t for no-tag labels once reviewed. The known bias travels with the prior: 10 cities and the ASSETS'24 reviewers' own
...
Crops to cut: 295,957 (tier 2 tag era), plus the 10,857 HF rows (train and test, re-cut at the
same framing, above), plus the expert-validate 2,452, minus overlap: at most 309,266
(arithmetic), about 48 GB at fov25's 155.0 KB. Store
...
|---|---:|---:|---|
| klone L40S, one arm per GPU (item 4, `gpu-l40s-makelab`) | 8,666 | 111.5–113.3 mean over 100 epochs (arms `fov25`, `fov50`, `viewport`) | `epoch_s` in `analysis_out/context_fov_86/train_<arm>_train_log.csv`, committed on `exp/context-fov-86` (PR #180); `train_s` 11,159–11,328 s per 100 epochs in `train_<arm>_train_meta.json` |
...
ran 3:58–4:57 h wall for 100 epochs including preprocessing and inference (14,317–17,824 s
elapsed, jobs 40486691–40486693, `docs/data/compute/sacct_klone_2026-09-24.txt` on
`exp/context-fov-86`).
...
1. **Presentation-matched, 3 epochs** (~3.2 h per arm on one L40S, plus prep). The same number
   ...
   reference rows is not compute; it is data *and* framing, since the reference rows are at the
   HF screenshot framing (§3.1). The matched `clean` control runs the recipe's 100 epochs
...
**10 % pano-grouped slice of the HF train rows that survive §2.4** (10 % of at most 4,275, so
...
`tag_benchmark_86.py train` was written for 8.7k crops and one sitting. PROPOSED (§8 decision
9): three changes, each a small PR against the benchmark script with its own test, before any
budget-2 job is submitted:
...
- **Preprocess once, outside the job.** The trainer decodes every crop into a uint8
  ...
  The 48 GB the item 4 jobs requested (`context_fov_86.slurm`) becomes `--mem=96G` for the
  memmap's page cache. The node memory that has to fit under is not in a committed file; the
  first prep job reads it from `scontrol show node` and writes it to the prep manifest.
- **Resume.** Save model, optimizer, epoch and the shuffle generator's state every epoch; on
  ...
  precondition for the `ckpt` partition, where `PreemptMode=REQUEUE`
  (`scripts/model_comparison/run_yolo_train.slurm`, header). The
...
**Allocation: not settled, a decision for Jon (§8 decision 14).** Two options:

- **`gpu-l40s-makelab`** (`gpu-l40s` has `MaxTime=UNLIMITED` and no preemption,
  `scripts/model_comparison/yolo_baseline/README.md`; one L40S per job, 8 CPUs, as
  `context_fov_86.slurm` requests). Predictable, but it is the lab's dedicated allocation, and
  the committed convention is that long runs stay off it so students keep it
  (`scripts/model_comparison/run_yolo_train.slurm`, header). Item 4 found one L40S free there and
  ran its four arms one after another (`context_fov_86.md` §5); at that rate budgets 1 and 2 plus
  `clean` (~75 GPU-h, §5.2) hold that GPU for about three days of wall-clock.
- **`ckpt`** (`-A ckpt-makelab`), the scavenger partition the convention points to. Free of the
  students' allocation, but preemptable (`PreemptMode=REQUEUE`), so it needs the resume change
  (§5.4) first, and a requeue costs the epoch in flight: at ~1.06 h per tier-2 epoch that is up
  to an hour per preemption. Throughput there has been poor before: one YOLO arm never finished
  an epoch inside a ckpt slice (`run_yolo_train.slurm`, lines 22–23).

PROPOSED: `ckpt` for the two 30-epoch arms once resume exists, since they are the long jobs the
convention is about; `gpu-l40s-makelab` for budget 1 and `clean` (about 11 GPU-h), which are
short enough not to crowd the students. One Slurm job per arm either way. Each job writes an
...
- **One seed per arm.** Same caveat as item 4; the benchmark's seed variance is unmeasured.
  PROPOSED: a second seed of `naive` and of `nnPU` at budget 1 (~6.4 GPU-h), which measures
  seed variance **at budget 1 only** and is reported as such; it does not bound the budget-2
  headline. Bounding that takes a second budget-2 seed of each arm, ~32 GPU-h apiece (§5.2),
  which is Jon's call once the budget-2 result is in (§8 decision 5). No fixed mAP threshold
  triggers it: the first draft's ~0.02 was the pano-leak bound of `tag_benchmark_86.md` §1, which
  says nothing about seed variance.
...
3. **Priors:** the HF train rows that survive the test exclusions (4,275 trainable) as the main prior
   (proposed; the all-10,857 rates include test rows and are shown for reference only),
   trusted-rater 2024+ as the sensitivity check, and the item-3 review's 150 `untagged` items,
   reweighted to the tier-2 pool, as a check on π_U,t for no-tag labels once reviewed?
4. **Arms:** `naive` + `nnPU` at both budgets, soft-negative (target π_U,t on untagged cells,
   §3.2) at budget 1 only?
5. **Budgets:** 3 epochs (presentation-matched) then 30 epochs, plus `clean`; ~75 GPU-h (~3
   GPU-days) for the full set, against the plan's ~2, on the allocation of decision 14? A
   second budget-2 seed (~32 GPU-h per arm) only if Jon asks for it after the budget-2 read (§7).
...
14. **Slurm allocation:** `ckpt` for the two 30-epoch arms once resume exists and
    `gpu-l40s-makelab` for budget 1 and `clean` (proposed), or everything on `gpu-l40s-makelab`
    (predictable, but ~3 days holding the students' L40S)? (§5.5)
