    ...
    if cfg.geometry == "tiles":
        ...
        for k, view in enumerate(cfg.views):
            ...
            if not lines and not _keep_background(cfg, stem):
                st["bg_skipped"] += 1
                continue
            view_img = equirect_to_perspective(src, view)
            _write_pair(cfg.out, yolo_split, stem, view_img, lines)
            st["tiles"] += 1
            st["boxes"] += len(lines)
            st["bg_tiles"] += 1 if not lines else 0
            if overlay and cfg.overlay_dir:
                _save_overlay(cfg.overlay_dir, stem, view_img, lines)
    else:  # pano
...
def write_data_yaml(cfg, args):
    strategy, fixed_frac = args.box_size
    box = f"fixed:{fixed_frac}" if strategy == "fixed" else strategy
    with open(os.path.join(cfg.out, "data.yaml"), "w") as f:
        # Stamp the full prep config as comments: box size et al. are train-only knobs
        # that no downstream record (ultralytics args.yaml) captures, so the dataset's
        # provenance must be answerable from disk.
        f.write("# Generated by prepare_yolo_dataset.py\n")
        f.write(f"# geometry={cfg.geometry} box-size={box} ramp-size-m={cfg.ramp_size_m} "
                f"camera-height-m={cfg.camera_height_m}\n")
        f.write(f"# min-frac={cfg.min_frac} max-frac={cfg.max_frac} "
                f"bg-keep-frac={cfg.bg_keep_frac} source-max-edge={cfg.source_max_edge}\n")
        f.write(f"# n-yaw={args.n_yaw} view-fov={args.view_fov} view-pitch={args.view_pitch} "
                f"view-size={args.view_size} pano-width={args.pano_width} "
                f"subset={args.subset} dataset-root={os.path.abspath(args.dataset_root)}\n")
...
def main():
    ...
    ap.add_argument("--overlay-n", type=int, default=20,
                    help="Panos to overlay PER SPLIT (default 20).")
    args = ap.parse_args()
