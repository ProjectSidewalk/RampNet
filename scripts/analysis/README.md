...
| `input_res_sweep_25.py extract` / `check` / `report` (+ `input_res_sweep_25.sh`) | **yes** (`extract` only) | **Frozen-model input-size sweep (#25, arm 1: no retraining).** Does the released checkpoint, unchanged, tolerate or gain from more input pixels? Six arms per pano (r2048 control, r3072, r4096, rnative capped at 5500x11000, the u4096 upsample control, and r4096 with a 1024x2048 heatmap), low-floor peak caches under `analysis_out/input_res_sweep_25/cache/<arm>/`; `check` asserts r2048 reproduces the committed `analysis_out/op_cache`; `report` gives P/R/F1 at 0.30 and 0.55, AP, headroom classes, recall by distance and apparent size, and paired pano-level bootstrap CIs, all on CPU from the committed caches. See [`docs/input_res_sweep_25.md`](../../docs/input_res_sweep_25.md). |
