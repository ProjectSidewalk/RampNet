...
| `overlap_test.py` | **yes** | Do the threshold and resolution levers target the same ramps? Writes per-GT-ramp hits at four thresholds to `analysis_out/overlap.json` (committed, #171); the cross-tab by distance additionally needs `gt_depth_da3.json`, which is not committed. |
