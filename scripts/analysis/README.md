...
| `tag_review_list.py build` / `power` | no | **The RampNet 2.0 tag review list (#86 item 3).** Draws the stratified, seeded 500-item list `benchmark/tag_review/review_list.csv` (tag state x distance band x city, rare tags up-weighted, one item per physical ramp) from PR #175's `ps_supervision_audit.py fetch` cache, and writes `review_list.meta.json` with every input's sha256. `power` prints the kappa-precision table the list size rests on. The rubric it serves is a **proposed draft**: [`docs/tag_rubric_draft.md`](../../docs/tag_rubric_draft.md). |
