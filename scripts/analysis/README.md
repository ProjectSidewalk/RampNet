...
| `train_overlap_check.py` | no | **Which reviewed benchmark panoramas are also in RampNet's training data? (#127)** Exact-id membership of every `BENCHMARK_SPLITS` split against `rampnet-dataset`'s train/validation splits, reading only the `pano_id` column of each Hub shard over HTTP range (network read-only, minutes). Writes the committed `benchmark/train_overlap.json`, which `export_benchmark.py` requires for every split it publishes and `score_validation.py --exclude-train-overlap` reads. |
...
| `input_res_sweep_25.py extract` / `check` / `report` / `sums` (+ `input_res_sweep_25.sh`) | **yes** (`extract` only) | **Frozen-model input-size sweep (#25, arm 1: no retraining).** Does the released checkpoint, unchanged, tolerate or gain from more input pixels? Six arms per pano (r2048 control, r3072, r4096, rnative capped at 5500x11000, the u4096 upsample control, and r4096 with a 1024x2048 heatmap), low-floor peak caches under `analysis_out/input_res_sweep_25/cache/<arm>/`; `check` asserts r2048 reproduces the committed `analysis_out/op_cache`; `report` gives P/R/F1 at 0.30 and 0.55, AP, headroom classes, recall by distance and apparent size, and paired pano-level bootstrap CIs, all on CPU from the committed caches; `sums` checks them and the caches against the committed `SHA256SUMS`. See [`docs/input_res_sweep_25.md`](../../docs/input_res_sweep_25.md). |
